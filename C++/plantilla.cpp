...
#define fast                 \
    ...
    cin.tie(0)
#define ms998                                            \
    for (int i = 0; i < 10e6 * 8 + 10e6 + 10e5 * 5; i++) \
    {                                                    \
        cnt++;                                           \
    }
#define inicio       \
...
ofstream out(strconcat(__FILE__,".out").c_str());
...
inline vT randarr(int _tsize_, int max_num)
{
    ...
    srand(time(0));
    max_num *= 100;
    generate(created_vector.begin(), created_vector.end(), [&]()
             { return (T)(rand() % max_num) / 100; });
...
int acmquery(vvi v, int i, int j, int ii, int jj)
{
    if (ii == 0 || jj == 0)
    {
...
inline int bsh_left(vector<T> __find_vector_, T _num_for_find_)
{
    if (_num_for_find_ > __find_vector_[__find_vector_.size() - 1])
        return __find_vector_.size() - 1;
...
inline vvi randmatrix(int n, int m, int modo)
{
    ...
    FOR(n)
    {
        generate(_rand_matrix[i].begin(), _rand_matrix[i].end(), [&]()
                 { return (int)(rand() % modo) / 100; });
    }
    return _rand_matrix;
}
