...
/*
    gcd(BigInt, BigInt)
    -------------------
    Returns the greatest common dv (GCD, a.k.a. HCF) of two BigInts using
    Euclid's algorithm.
*/
...
/*
    divide
    ------
    Helper function that returns the quor and remainder on dividing the
    dividend by the dv, when the dv is 1 to 10 times the dividend.
*/

std::tuple<BigInt, BigInt> divide(const BigInt &dividend, const BigInt &dv)
{
    BigInt quor, remainder, temp;
    ...
    {
        quor--;
        remainder = dividend - (temp - dv);
    }

    return std::make_tuple(quor, remainder);
}

/*
    BigInt / BigInt
    ---------------
    Computes the quor of two BigInts using the long-division method.
    The operand on the RHS of the division (the dv) is `num`.
*/

BigInt BigInt::operator/(const BigInt &num) const
{
    BigInt avdd = abs(*this);
    BigInt davd = abs(num);
    ...
    BigInt quor;
    if (avdd <= LLONG_MAX and davd <= LLONG_MAX)
        quor = std::stoll(avdd.value) / std::stoll(davd.value);
    else if (avdd == davd)
        quor = 1;
    else if (is_power_of_10(davd.value))
    { // if dv is a power of 10 do optimised calculation
        size_t digits_in_quotient = avdd.value.size() - davd.value.size() + 1;
        quor.value = avdd.value.substr(0, digits_in_quotient);
    }
    else
    {
        quor.value = ""; // the value is cleared as digits will be appended
        BigInt chunk, chunk_quotient, chunk_remainder;
        size_t chunk_index = 0;
        chunk_remainder.value = avdd.value.substr(chunk_index, davd.value.size() - 1);
        chunk_index = davd.value.size() - 1;
        while (chunk_index < avdd.value.size())
        {
            chunk.value = chunk_remainder.value.append(1, avdd.value[chunk_index]);
            chunk_index++;
            while (chunk < davd)
            {
                quor.value += "0";
                if (chunk_index < avdd.value.size())
                {
                    chunk.value.append(1, avdd.value[chunk_index]);
                    chunk_index++;
                }
                else
                    break;
            ...
            else if (chunk > davd)
            {
                ...
                std::tie(chunk_quotient, chunk_remainder) = divide(chunk, davd);
                quor.value += chunk_quotient.value;
            }
        }
    }
    strip_leading_zeroes(quor.value);
...
/*
    ...
    The operand on the RHS of the modulo (the dv) is `num`.
*/
...
BigInt BigInt::operator%(const BigInt &num) const
{
    ...
    if (davd == 0)
        throw std::logic_error("Attempted division by zero");
    if (davd == 1 or davd == avdd)
        return BigInt(0);

    BigInt remainder;
    if (avdd <= LLONG_MAX and davd <= LLONG_MAX)
        remainder = std::stoll(avdd.value) % std::stoll(davd.value);
    else if (avdd < davd)
        remainder = avdd;
    else if (is_power_of_10(num.value))
    { // if num is a power of 10 use optimised calculation
        size_t no_of_zeroes = num.value.size() - 1;
        remainder.value = avdd.value.substr(avdd.value.size() - no_of_zeroes);
    }
    else
    {
        BigInt quor = avdd / davd;
        remainder = avdd - quor * davd;
    }
